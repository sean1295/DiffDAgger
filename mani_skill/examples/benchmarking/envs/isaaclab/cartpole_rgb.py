...
class CartpoleRGBCameraBenchmarkEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(
        num_envs=256, env_spacing=20.0, replicate_physics=True
    )
...
class CartpoleCameraBenchmarkEnv(DirectRLEnv):
    ...
    def _configure_gym_env_spaces(self):
        ...
        if self.has_rgb:
            self.single_observation_space["rgb"] = gym.spaces.Box(
                ...
                shape=(
                    self.num_cameras,
                    self.tiled_rgb_camera_cfgs[0].height,
                    self.tiled_rgb_camera_cfgs[0].width,
                    3,
                ),
            )
        if self.has_depth:
            self.single_observation_space["depth"] = gym.spaces.Box(
                ...
                shape=(
                    self.num_cameras,
                    self.tiled_depth_camera_cfgs[0].height,
                    self.tiled_depth_camera_cfgs[0].width,
                    1,
                ),
            )
    ...
    def _setup_scene(self):
        ...
        if self.has_rgb:
            self.tiled_rgb_cameras = [
                TiledCamera(cfg) for cfg in self.tiled_rgb_camera_cfgs
            ]
        if self.has_depth:
            self.tiled_depth_cameras = [
                TiledCamera(cfg) for cfg in self.tiled_depth_camera_cfgs
            ]
        ...
        if self.has_depth:
            for i in range(self.num_cameras):
                self.scene.sensors[f"tiled_depth_camera_{i}"] = (
                    self.tiled_depth_cameras[i]
                )
        # add lights
    ...
    def _get_observations(self) -> dict:
        ...
        if self.has_rgb:
            for i, (cam, cfg) in enumerate(
                zip(self.tiled_rgb_cameras, self.tiled_rgb_camera_cfgs)
            ):
                observations["sensors"][f"cam_{i}"]["rgb"] = cam.data.output[
                    "rgb"
                ].clone()
        if self.has_depth:
            for i, (cam, cfg) in enumerate(
                zip(self.tiled_depth_cameras, self.tiled_depth_camera_cfgs)
            ):
                observations["sensors"][f"cam_{i}"]["depth"] = cam.data.output[
                    "depth"
                ].clone()
        return observations
