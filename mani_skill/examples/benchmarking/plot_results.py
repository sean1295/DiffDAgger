...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Paths to the benchmark result files to be plotted",
    )
    return parser.parse_args()


COLOR_PALLETE = ["#e02b35", "#59a89c", "#082a54", "#a559aa" "#f0c571"]
...
def main(args):
    ...
    for cam_size in cam_sizes:
        ...
        ax.grid(True)
        ax.set_title(
            f"{args.env_id}: RGB FPS vs Number of Parallel Envs. 1x{cam_size}x{cam_size} RGB Camera"
        )
        ...
        for i, (exp_name, df) in enumerate(data.items()):
            df = df[
                (df["obs_mode"] == "rgb")
                & (df["camera_width"] == cam_size)
                & (df["camera_height"] == cam_size)
                & (df["num_cameras"] == 1)
                & (df["env.step/gpu_mem_use"] < 16 * 1024 * 1024 * 1024)
            ]
            if len(df) == 0:
                continue
            ...
            for j, (x, y) in enumerate(zip(df["num_envs"], df["env.step/fps"])):
                ax.annotate(
                    f'{df["env.step/gpu_mem_use"].iloc[j] / (1024 * 1024 * 1024):0.1f} GB',
                    (x, y),
    ...
    ax.set_ylabel("FPS")
    ax.set_title(
        f"{args.env_id}: Highest RGB FPS vs Number of 128x128 Cameras under 16GB GPU memory"
    )
    for i, (exp_name, df) in enumerate(data.items()):
        df = df[df["env.step/gpu_mem_use"] < 16 * 1024 * 1024 * 1024]
        df = df[(df["obs_mode"] == "rgb")]
    ...
    for i, (exp_name, df) in enumerate(data.items()):
        ...
        for j, (x_val, y_val, mem_use) in enumerate(
            zip(x, df["env.step/fps"], df["env.step/gpu_mem_use"])
        ):
            ax.annotate(
