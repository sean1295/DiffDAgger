...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Number of cameras. Only used by benchmark environments",
)
parser.add_argument(
    "--cam-width",
    type=int,
    default=None,
    help="Width of cameras. Only used by benchmark environments",
)
parser.add_argument(
    "--cam-height",
    type=int,
    default=None,
    help="Height of cameras. Only used by benchmark environments",
)
parser.add_argument(
...
assert (
    args_cli.obs_mode != "rgbd"
), "IsaacLab currently does not support rendering RGB + Depth"
...
def main():
    ...
    with torch.inference_mode():
        ...
        with profiler.profile(
            "env.step+env.reset", total_steps=N, num_envs=args_cli.num_envs
        ):
            for i in range(N):
                actions = (
                    2 * torch.rand(env.action_space.shape, device=env.unwrapped.device)
    ...
    if args_cli.obs_mode in ["rgb", "rgbd", "depth"]:
        ...
        for i in range(args_cli.num_cams):
            ...
            sensor_settings_str.append(
                f"{cam_type}({args_cli.cam_width}x{args_cli.cam_height})"
            )
