...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="panda",
        help="The id of the robot to place in the environment",
    )
    parser.add_argument(
        "-b",
        "--sim-backend",
    ...
    parser.add_argument(
        ...
        help="The control mode to use. Note that for new robots being implemented if the _controller_configs is not implemented in the selected robot, we by default provide two default controllers, 'pd_joint_pos' and 'pd_joint_delta_pos' ",
    )
    parser.add_argument(
        "-k",
        "--keyframe",
        type=str,
        help="The name of the keyframe of the robot to display",
    )
    parser.add_argument(
        "--shader",
        default="default",
    ...
    parser.add_argument(
        "--keyframe-actions",
        action="store_true",
        help="Whether to use the selected keyframe to set joint targets to try and hold the robot in its position",
    )
    parser.add_argument(
        "--random-actions",
        action="store_true",
        help="Whether to sample random actions to control the agent. If False, no control signals are sent and it is just rendering.",
    )
    parser.add_argument(
        "-s",
        "--seed",
...
def main():
    ...
    while True:
        if args.random_actions:
            ...
            assert (
                kf is not None
            ), "this robot has no keyframes, cannot use it to set actions"
